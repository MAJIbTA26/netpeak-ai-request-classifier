...
def test_classify_error_recognizes_rate_limit():
    ...
    assert classify_error(Exception(error_message)) == "rate_limit"
...
def test_classify_error_recognizes_temporary_unavailable():
    ...
    assert classify_error(Exception(error_message)) == "temporary_unavailable"
...
def test_classify_error_recognizes_model_not_found():
    ...
    assert classify_error(Exception(error_message)) == "model_not_found"


def test_classify_error_recognizes_auth_error():
    """Перевіряємо, що помилка авторизації розпізнається окремо."""
    error_message = "401 UNAUTHENTICATED. Invalid API key provided."
    assert classify_error(Exception(error_message)) == "auth_error"


def test_classify_error_falls_back_to_unknown():
    """Перевіряємо, що невідомі помилки отримують категорію unknown,
    а не викликають виняток самої функції класифікації."""
    error_message = "Якась зовсім нова, незнайома помилка"
    assert classify_error(Exception(error_message)) == "unknown"
...
def test_parse_retry_delay_returns_default_when_no_delay_found():
    """Перевіряємо, що функція повертає значення за замовчуванням,
    якщо у тексті немає підказки про час."""
